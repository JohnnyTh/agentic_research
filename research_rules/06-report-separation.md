...
it imports from a `reports/report_components_NN_name/` package and calls, in order,
`collect` (if present) → `compute` → `render`. It must not itself compute
stats or build HTML strings.

- **`reports/report_components_NN_name/compute.py`** — pure functions, input data in, a stats
  dict out. No HTML, no network/file I/O beyond reading its own inputs.
- **`reports/report_components_NN_name/render.py`** — pure functions, stats dict in, HTML (or
  Markdown) string out. No stats computation, no network/file I/O.
- **`reports/report_components_NN_name/collect.py`** — optional, only when the report needs its
...
`compute.py`/`render.py`/`collect.py` within one report's own `report_components_NN_name/`
